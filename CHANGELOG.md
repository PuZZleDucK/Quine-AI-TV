...
- 2026-02-15 21:20 (Australia/Melbourne) [project:quine-tv] `oceanpostcards` (src/channels/archive/oceanpostcards.js): text legibility — improved UI label readability (header/title, creature name/zone, “CREATURE FACT”, “NEXT CARD”) with subtle shadow/outline + slightly darker ink palette. Commit: 8088cd0

- 2026-02-15 21:07 (Australia/Melbourne) [project:quine-tv] Review channel `oceanpostcards` (src/channels/archive/oceanpostcards.js): screenshots 0–300s `screenshots/review-oceanpostcards` (errors/warnings: 0); confirmed `// REVIEWED: 2026-02-15`; queued concrete follow-ups in TODO. Commit: TBD
...
- 2026-02-15 15:19 (Australia/Melbourne) [project:quine-tv] `traveldesk` (src/channels/traveldesk.js): street feed variety — added occasional foreground pass sprites (streetlight pole / bus silhouette / neon sign / moon) with faster parallax so the inset feels less static over minutes. Commit: 8f05e65
...
- 2026-02-15 03:05 (Australia/Melbourne) [project:quine-tv] `tidydesk` (src/channels/tidydesk.js): determinism — fixed-timestep sim loop (`SIM_DT=1/60` accumulator) so 30fps/60fps captures match. Commit: f078c95

- 2026-02-15 02:33 (Australia/Melbourne) [project:quine-tv] `tidydesk` (src/channels/tidydesk.js): variety — seeded rotating subtitle+footer text pools (45s cadence; 8 lines; 6+ minutes before repeat). Commit: 7245244

- 2026-02-15 02:17 (Australia/Melbourne) [project:quine-tv] `tidydesk` (src/channels/tidydesk.js): perf — compute `deskRect()` once per render (pass `d` into draw helpers) instead of recomputing per item. Commit: c649d51

- 2026-02-15 02:00 (Australia/Melbourne) [project:quine-tv] `tidydesk` (src/channels/tidydesk.js): perf — cached desk background gradients (bg/wood/vignette) (rebuild on resize/ctx swap) so steady-state `render()` allocates 0 gradients/frame. Commit: d3e98a9

- 2026-02-15 01:54 (Australia/Melbourne) [project:quine-tv] Review channel `tidydesk` (src/channels/tidydesk.js): captured screenshots (0–300s) to `screenshots/review-tidydesk` + completion shots to `screenshots/review-tidydesk-2026-02-15-post` (errors/warnings: 0), added `// REVIEWED: 2026-02-15`, and fixed audio determinism/hygiene by splitting an `audioRand` PRNG and making `onAudioOn()` idempotent (audio no longer consumes the visual RNG sequence). Commit: TBD

- 2026-02-15 01:21 (Australia/Melbourne) [project:quine-tv] `micromyst` (src/channels/micromystery.js): determinism — removed `rand()` usage from `render()` via deterministic (seeded) time-bucketed grain variants + per-line jitter hash (FPS-stable render). Commit: 018201a
...
- 2026-02-14 21:30 (Australia/Melbourne) [project:quine-tv] `mailroomtube` (src/channels/mailroomtube.js): text — expanded `DISPATCH_LOG` to 96 lines (~16 minutes rotation) and added deterministic jam/sweep override message for the HUD dispatch strip (no per-frame RNG consumption). Commit: 5b70c3f
...
- 2026-02-14 20:35 (Australia/Melbourne) [project:quine-tv] `mailroomtube` (src/channels/mailroomtube.js): text — added a seeded rotating “dispatch log” strip in the HUD (OSD-safe; ~6 minutes before repeating). Commit: f70f68d
...
- 2026-02-14 18:46 (Australia/Melbourne) [project:quine-tv] `lasercutfile` (src/channels/lasercutfile.js): perf — cached the inner bed gradient created in `drawBed()` (rebuild on resize/ctx swap) so steady-state render avoids per-frame `createLinearGradient()`. Commit: c952b51
...
- 2026-02-14 16:05 (Australia/Melbourne) [project:quine-tv] `packetsfm` (src/channels/packetsnifferfm.js): determinism — switched packet spawning + waterfall updates to a fixed-timestep simulation loop (`SIM_DT=1/60`) so RNG consumption is FPS-stable (30fps/60fps captures match). Commit: dd311eb

- 2026-02-14 15:51 (Australia/Melbourne) [project:quine-tv] `packetsfm` (src/channels/packetsnifferfm.js): text/dialog — added a seeded rotating “packet log” / callout strip (~5.6s cadence; ~6 minutes before repeating), clipped OSD-safe inside the waterfall panel. Commit: cbaddfa
...
- 2026-02-14 12:17 (Australia/Melbourne) [project:quine-tv] `news` (src/channels/news.js): OSD safety — constrained the bouncing ODD NEWS logo so it won’t overlap the top-left LIVE/time bug (safe rectangle + push-out). Screenshots: screenshots/autopilot-news-osd-before + screenshots/autopilot-news-osd-after. Commit: da5aa95
...
- 2026-02-14 08:49 (Australia/Melbourne) [project:quine-tv] `microfilm` (src/channels/microfilm.js): perf — cached static background + vignette into offscreen layers rebuilt on init/resize (steady-state render blits; no per-frame backdrop gradients). Commit: ffb48cb

- 2026-02-14 08:35 (Australia/Melbourne) [project:quine-tv] `microfilm` (src/channels/microfilm.js): audio hygiene — made `onAudioOn()` idempotent and ensured `onAudioOff()`/`destroy()` only clear AudioManager.current when owned. Screenshots: screenshots/autopilot-microfilm-audio-hygiene/before + screenshots/autopilot-microfilm-audio-hygiene/after. Commit: 3b58ccf
